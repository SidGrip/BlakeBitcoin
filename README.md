<p align="center">
  <img src="src/qt/res/icons/bitcoin.png" alt="BlakeBitcoin" width="95">
</p>

# BlakeBitcoin 0.15.21

> SegWit signaling schedule: BlakeBitcoin mainnet versionbits signaling starts on May 11, 2026 00:00:00 UTC (`1778457600`) and times out on May 11, 2027 00:00:00 UTC (`1809993600`). Activation still depends on BIP9 lock-in.

## About BlakeBitcoin

BlakeBitcoin is a Blake-256 cryptocurrency in the BlakeStream family. This repository carries the BlakeBitcoin Core `0.15.21` update and ships build/release packaging for native Ubuntu `20.04`, `22.04`, `24.04`, `25.10`, Windows, native macOS, and Ubuntu `22.04+` AppImage.
...
| Difficulty retarget | Every 8,064 blocks (14 days) |
| Default port | 8356 |
| RPC port | 8243 |
| Max supply | 21,000,000 BBTC |
...
- The output folder keeps `BlakeBitcoin-0.15.21-x86_64.AppImage`, `README.md`, and `build-info.txt`
...
- Writes standalone `.exe` outputs plus `build-info.txt` to `outputs/Windows/`
...
outputs/
├── AppImage/
│   ├── BlakeBitcoin-0.15.21-x86_64.AppImage
│   ├── README.md
│   └── build-info.txt
├── Macosx/
│   ├── BlakeBitcoin-Qt.app
│   ├── blakebitcoin-cli-0.15.21
│   ├── blakebitcoin-qt-0.15.21
│   ├── blakebitcoin-tx-0.15.21
│   ├── blakebitcoind-0.15.21
│   └── build-info.txt
├── Ubuntu-20/
│   ├── README.md
...
For native macOS builds, the current host's daemon tools, `BlakeBitcoin-Qt.app`, and the raw `blakebitcoin-qt-0.15.21` binary all land in `outputs/Macosx/`.

For AppImage builds, `outputs/AppImage/` keeps `BlakeBitcoin-0.15.21-x86_64.AppImage`, `README.md`, and `build-info.txt`.
...
BlakeBitcoin is released under the terms of the MIT license. See `COPYING` for more information.
