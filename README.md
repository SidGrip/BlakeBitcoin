...
BlakeBitcoin was inspired and requested by the Blake community. It is a Blake-256 version of Bitcoin with the same reward structure and difficulty algorithm, but with a block target time 1/4 of Bitcoin's to account for the extra hashing speed of Blake-256.
...
- AuxPow-enabled — merge mined with Blakecoin and Photon
...
- Builds both the daemon (`blakebitcoind`) and Qt wallet (`blakebitcoin-qt`) natively on Ubuntu 18.04
...
Targets:
  --daemon          Daemon only (blakebitcoind)
  --qt              Qt wallet only (blakebitcoin-qt)
  --both            Both (default)
...
```
outputs/
├── blakebitcoin.conf   Auto-generated config with RPC credentials and peers
├── native/
│   ├── daemon/         blakebitcoind
│   └── qt/             blakebitcoin-qt
├── linux-appimage/
│   └── qt/             BlakeBitcoin-x86_64.AppImage
├── windows/
│   ├── daemon/
│   └── qt/             blakebitcoin-qt.exe
└── macos/
    ├── daemon/         blakebitcoind
    └── qt/             BlakeBitcoin-Qt.app
```

The config file is auto-generated on first build with random RPC credentials, active peers from the network, and default settings. Copy it to `~/.blakebitcoin/blakebitcoin.conf` before running the daemon.
