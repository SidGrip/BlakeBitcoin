...
static std::map<std::string, std::shared_ptr<CBlock>> mapAuxBlocksByScript;
static std::map<uint256, std::shared_ptr<CBlock>> mapAuxBlocksByHash;
static std::map<uint256, std::shared_ptr<CReserveScript>> mapAuxReserveScripts;
static std::vector<std::unique_ptr<CBlockTemplate>> vAuxBlockTemplates;
...
UniValue generateBlocks(std::shared_ptr<CReserveScript> coinbaseScript, int nGenerate, uint64_t nMaxTries, bool keepScript)
{
    ...
    while (nHeight < nHeightEnd)
    {
        ...
        while (nMaxTries > 0 && pblock->nNonce < nInnerLoopCount && !CheckAuxPowProofOfWork(*pblock, Params().GetConsensus())) {
            ++pblock->nNonce;
            --nMaxTries;
        }
        if (nMaxTries == 0) {
...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    ...
    coinbaseScript->reserveScript = GetScriptForDestination(address);
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Node is downloading blocks...");
...
static UniValue AuxMiningCreateBlock(const CScript& scriptPubKey, const std::shared_ptr<CReserveScript>& reserveScript = nullptr)
{
    ...
    const std::string scriptKey = HexStr(scriptPubKey.begin(), scriptPubKey.end());
    auto scriptIt = mapAuxBlocksByScript.find(scriptKey);
    if (scriptIt != mapAuxBlocksByScript.end()) {
        pblock = scriptIt->second;
    }

    const CBlockIndex* pindexNew = nullptr;
    {
        LOCK(cs_main);
        pindexNew = chainActive.Tip();

        if (!pblock || pindexPrev != pindexNew ||
            (mempool.GetTransactionsUpdated() != nTransactionsUpdatedLast && GetTime() - nStart > 60)) {
            if (pindexPrev != pindexNew) {
                mapAuxBlocksByScript.clear();
                mapAuxBlocksByHash.clear();
                mapAuxReserveScripts.clear();
                vAuxBlockTemplates.clear();
                pblock.reset();
            }
            ...
            mapAuxBlocksByScript[scriptKey] = pblock;
            mapAuxBlocksByHash[pblock->GetHash()] = pblock;
            vAuxBlockTemplates.push_back(std::move(newBlock));
        }
    }

    if (!pblock || !pindexPrev) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Unable to create AuxPoW block");
    }

    if (reserveScript) {
        mapAuxReserveScripts[pblock->GetHash()] = reserveScript;
    }
...
static UniValue AuxMiningSubmitBlock(const uint256& hash, const CAuxPow& auxpow)
{
    ...
    {
        LOCK(cs_auxpowrpc);
        auto it = mapAuxBlocksByHash.find(hash);
        if (it == mapAuxBlocksByHash.end()) {
...
static void KeepAuxMiningScript(const uint256& hash)
{
    LOCK(cs_auxpowrpc);
    auto it = mapAuxReserveScripts.find(hash);
    if (it != mapAuxReserveScripts.end()) {
        if (it->second) {
            it->second->KeepScript();
        }
        mapAuxReserveScripts.erase(it);
    }
}
...
UniValue createauxblock(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. address      (string, required) coinbase transaction payout address\n"
            "\nResult:\n"
            "{\n"
    ...
    if (!IsValidDestination(address)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid coinbase payout address");
    }

    return AuxMiningCreateBlock(GetScriptForDestination(address));
}
...
UniValue submitauxblock(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2) {
        throw std::runtime_error(
            ...
            "\nSubmit a solved auxpow for a block created by createauxblock.\n"
            "\nArguments:\n"
            "1. hash      (string, required) hash of the block to submit\n"
            "2. auxpow    (string, required) serialized auxpow found\n"
            "\nResult:\n"
            "true|false   (boolean) whether the submitted block was accepted\n"
            "\nExamples:\n"
            + HelpExampleCli("submitauxblock", "\"hash\" \"serialized auxpow\"")
            + HelpExampleRpc("submitauxblock", "\"hash\" \"serialized auxpow\""));
    }
...
UniValue getauxblock(const JSONRPCRequest& request)
{
    if (request.fHelp || (request.params.size() != 0 && request.params.size() != 2)) {
        throw std::runtime_error(
            ...
            "\nWithout arguments, create a new block using the wallet mining keypool.\n"
            "With arguments, submit a solved auxpow for a previously returned block.\n"
            "\nArguments:\n"
            "1. hash      (string, optional) hash of the block to submit\n"
            "2. auxpow    (string, optional) serialized auxpow found\n"
    ...
    if (request.params.empty()) {
        ...
        return AuxMiningCreateBlock(coinbaseScript->reserveScript, coinbaseScript);
#else
        throw JSONRPCError(RPC_METHOD_NOT_FOUND, "getauxblock without arguments requires wallet support");
#endif
    }
