...
public:
    ...
    void retranslateUi(QDialog *SendCoinsDialog)
    {
...
#if QT_CONFIG(tooltip)
        checkBoxMinimumFee->setToolTip(QCoreApplication::translate("SendCoinsDialog", "Paying only the minimum fee is just fine as long as there is less transaction volume than space in the blocks. But be aware that this can end up in a never confirming transaction once there is more demand for bitcoin transactions than the network can process.", nullptr));
#endif // QT_CONFIG(tooltip)
        checkBoxMinimumFee->setText(QString());
#if QT_CONFIG(tooltip)
        labelMinFeeWarning->setToolTip(QCoreApplication::translate("SendCoinsDialog", "Paying only the minimum fee is just fine as long as there is less transaction volume than space in the blocks. But be aware that this can end up in a never confirming transaction once there is more demand for bitcoin transactions than the network can process.", nullptr));
#endif // QT_CONFIG(tooltip)
        ...
        optInRBF->setText(QCoreApplication::translate("SendCoinsDialog", "Request Replace-By-Fee", nullptr));
#if QT_CONFIG(tooltip)
        optInRBF->setToolTip(QCoreApplication::translate("SendCoinsDialog", "Indicates that the sender may wish to replace this transaction with a new one paying higher fees (prior to being confirmed).", nullptr));
#endif // QT_CONFIG(tooltip)
#if QT_CONFIG(tooltip)
