...
const QString BITCOIN_IPC_PREFIX("blakebitcoin:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the blakebitcoin: URI contains a payment request, we are not able to detect the
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click blakebitcoin: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start blakebitcoin: click-to-pay handler"));
        }
        else {
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in blakebitcoin: URIs
    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be caused by an invalid BlakeBitcoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
